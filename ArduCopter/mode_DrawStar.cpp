 ...
 * 五角星航线模式初始化
 */
bool ModeDrawStar::init(bool ignore_checks)  //初始化
{
...
// 生成五角星航线
void ModeDrawStar::generate_path()  //自定义的生成航点函数
{
    float radius_cm = g2.star_radius_cm;

    wp_nav->get_wp_stopping_point(path[0]);  //停止点作为第0个航点

    path[1] = path[0] + Vector3f(1.0f, 0, 0) * radius_cm;  //第1个航点  radians 角度值转化为弧度值 cosf 浮点型的cos值（库）
...
// 开始位置控制
void ModeDrawStar::pos_control_start()  //开始位置控制
{
    // initialise waypoint and spline controller
    wp_nav->wp_and_spline_init();  //航点导航库的初始化

    // no need to check return status because terrain data is not used
    wp_nav->set_wp_destination(path[0], false);  //将当前目标航点设置为第0点

    // initialise yaw
    auto_yaw.set_mode_to_default(false);  //航向控制设置为默认
}
